...
struct Has<T, D, true> : std::false_type {};

template<typename T, typename D>
struct Has<T, D, false> {
    static constexpr bool value = std::is_same_v<front<D>, T> || Has<T, pop_front<D>>::value;
};
...
//  ------------------------------------------------------------------------------------------
    ...
    static_assert(has_v<double, Deque<float, double>>);
    static_assert(!has_v<char, Deque<int>>);
}
